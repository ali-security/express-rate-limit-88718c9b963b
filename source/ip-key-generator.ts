...
// The IPv4-mapped range (RFC 4291, section 2.5.5.2). Membership in it is decided
// from the address bits rather than from its notation.
const ipv4MappedSubnet = new Address6('::ffff:0:0/96')
// The deprecated IPv4-compatible range (RFC 4291, section 2.5.5.1).
...
export function ipKeyGenerator(ip: string, ipv6Subnet: number | false = 56) {
	if (isIPv6(ip)) {
		...
		//
		// Whether an address is mapped has to be decided from its bits (membership
		// in `ipv4MappedSubnet`) rather than from its notation (`is4()`), because
		// those are different questions: `::ffff:1.2.3.4` and `::ffff:102:304` are
		// one address written two ways and must produce one key, while
		// `2001:db8::1.2.3.4` is an ordinary IPv6 address that merely ends in
		// dotted-quad notation. Extracting `1.2.3.4` from the latter would skip the
		// subnet entirely and key the client on the bits it controls itself, as well
		// as put it in the same bucket as the unrelated IPv4 client 1.2.3.4.
		if (
			address.isInSubnet(ipv4MappedSubnet) ||
